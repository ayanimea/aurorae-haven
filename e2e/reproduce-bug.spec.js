...
test('Verify routine templates are created as routines, not tasks', async ({
  page,
  context
}, testInfo) => {
  ...
  const handleUseTemplateLogs = logs.filter(
    (l) => l.includes('handleUseTemplate') || l.includes('Template type:')
  )
